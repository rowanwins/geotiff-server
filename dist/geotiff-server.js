...
class Headers {
  ...
  */
	get(name) {
		name = `${name}`;
		validateName(name);
		const key = find(this[MAP$1], name);
		if (key === undefined) {
			return null;
		}
  ...
  */
	has(name) {
		name = `${name}`;
		validateName(name);
		return find(this[MAP$1], name) !== undefined;
	}
...
function exportNodeCompatibleHeaders(headers) {
	...
	const hostHeaderKey = find(headers[MAP$1], 'Host');
...
function latLonToUtm(coords, zone) {
  ...
  var latBand = mgrsLatBands.charAt(Math.floor(lat / 8 + 10)); // adjust zone & central meridian for Norway
  ...
  var a = 6378137;
  var f = 1 / 298.257223563; // WGS 84: a = 6378137, b = 6356752.314245, f = 1/298.257223563;
  ...
  1 / 2 * n - 2 / 3 * n2 + 5 / 16 * n3 + 41 / 180 * n4 - 127 / 288 * n5 + 7891 / 37800 * n6, 13 / 48 * n2 - 3 / 5 * n3 + 557 / 1440 * n4 + 281 / 630 * n5 - 1983433 / 1935360 * n6, 61 / 240 * n3 - 103 / 140 * n4 + 15061 / 26880 * n5 + 167603 / 181440 * n6, 9561 / 161280 * n4 - 179 / 168 * n5 + 6601661 / 7257600 * n6, 34729 / 80640 * n5 - 3418889 / 1995840 * n6, 212378941 / 319334400 * n6];
  var ξ = ξʹ;
  var j = 1;
...
var LandsatPdsProvider =
/*#__PURE__*/
function () {
  function LandsatPdsProvider() {
    _classCallCheck(this, LandsatPdsProvider);
  ...
  _createClass(LandsatPdsProvider, [{
    key: "constructImageUrl",
    value: function constructImageUrl(sceneId, band) {
      var path = sceneId.substring(3, 6);
      var row = sceneId.substring(6, 9);
      return "".concat(this.baseUrl, "/").concat(path, "/").concat(row, "/").concat(sceneId, "/").concat(sceneId, "_").concat(band.filePath, ".TIF");
    }
  }, {
    key: "getBandByShortName",
    value: function getBandByShortName(shortName) {
      for (var i = 0; i < this.bands.length; i++) {
        if (this.bands[i].shortName === shortName) return this.bands[i];
      }
    }
  }, {
    key: "getRequiredBandsByShortNames",
    value: function getRequiredBandsByShortNames(bands, sceneId) {
      var requiredBands = [];

      for (var i = 0; i < bands.length; i++) {
        var matchingBand = this.getBandByShortName(bands[i]);
    ...
    key: "getBandUrls",
    value: function getBandUrls(sceneId, bands) {
      var urls = [];
    ...
    value: function () {
      ...
      /*#__PURE__*/
      regeneratorRuntime.mark(function _callee(sceneId) {
        var path, row, metaUrl, res, meta;
        return regeneratorRuntime.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                path = sceneId.substring(3, 6);
                row = sceneId.substring(6, 9);
                metaUrl = "".concat(this.baseUrl, "/").concat(path, "/").concat(row, "/").concat(sceneId, "/").concat(sceneId, "_MTL.json");
                _context.next = 5;
    ...
    key: "reprojectBbbox",
    value: function reprojectBbbox(requestBbox) {
      var sceneUtmZone = this.metadata.L1_METADATA_FILE.PROJECTION_PARAMETERS.UTM_ZONE;
      var bboxMinUtm = latLonToUtm([requestBbox[0], requestBbox[1]], sceneUtmZone);
      var bboxMaxUtm = latLonToUtm([requestBbox[2], requestBbox[3]], sceneUtmZone);
      return [bboxMinUtm[0], bboxMinUtm[1], bboxMaxUtm[0], bboxMaxUtm[1]];
    }
  }, {
    key: "performToa",
    value: function performToa(band) {
      var sunElevation = this.metadata.L1_METADATA_FILE.IMAGE_ATTRIBUTES.SUN_ELEVATION;
      var se = Math.sin(degreesToRadians(sunElevation));
      var reflectanceRescalingFactor = this.metadata.L1_METADATA_FILE.RADIOMETRIC_RESCALING["REFLECTANCE_MULT_BAND_".concat(band.bandNumber)];
      var reflectanceAddition = this.metadata.L1_METADATA_FILE.RADIOMETRIC_RESCALING["REFLECTANCE_ADD_BAND_".concat(band.bandNumber)];
  ...
  return LandsatPdsProvider;
}();

var providerList = [new LandsatPdsProvider()];
...
function getRampByName(name) {
  if (name === 'NDWI') return createNdwi();
}
function getRgbFromRamp(colorRamp, value) {
...
function createSingleBandTile(bands, expression, styleName) {
  var parser = new Parser();
  var expr = parser.parse(expression);
  var colorRamp = getRampByName(styleName);
  ...
  for (var i = 0; i < frameData.length / 4; i++) {
    ...
    var rgb = getRgbFromRamp(colorRamp, calculatedVal);
    frameData[i * 4] = rgb[0];
...
function getScene(_x, _x2, _x3) {
  return _getScene.apply(this, arguments);
}

function _getScene() {
  _getScene = _asyncToGenerator(
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee(band, bbox, provider) {
    var tiff, data;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.next = 2;
            return GeoTIFF.fromUrl(band.urlPath);
          ...
          case 5:
            ...
            return _context.abrupt("return", band);
...
var rgbTiles = /*#__PURE__*/
(function () {
  var _ref = _asyncToGenerator(
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee(req, res) {
    var sceneId, providerSrc, provider, requestBbox, imgBbox, requiredBandsShortNames, bandsToUse, getDataCalls, i, _ref2, _ref3, r, g, b, png, img;

    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            sceneId = req.query.sceneId ? req.query.sceneId : null;
            ...
            throw 'GeoTiff-Server Error: You must pass in a sceneId to your query';
          ...
          case 7:
            provider.metadata = _context.sent;
            requestBbox = createBbox(Number(req.params.x), Number(req.params.y), Number(req.params.z));
            imgBbox = requestBbox;
            ...
            bandsToUse = provider.getRequiredBandsByShortNames(requiredBandsShortNames, sceneId);
            getDataCalls = [];

            for (i = 0; i < bandsToUse.length; i++) {
          ...
          case 17:
            ...
            g = _ref3[1];
            b = _ref3[2];
            png = createRgbTile(r.data, g.data, b.data);
...
var calculate = /*#__PURE__*/
(function () {
  var _ref = _asyncToGenerator(
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee(req, res) {
    var sceneId, providerSrc, provider, requestBbox, imgBbox, style, ratio, requiredBandsShortNames, bandsToUse, getDataCalls, i, data, png, img;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            throw 'GeoTiff-Server: You must pass in a sceneId to your query';
          ...
          case 7:
            ...
            imgBbox = requestBbox;
            if (provider.requiresReprojecting) imgBbox = provider.reprojectBbbox(requestBbox);
            style = req.query.style ? req.query.style : 'NDWI';
          ...
          case 19:
            data = _context.sent;
            png = createSingleBandTile(data, ratio, style);
            img = Buffer.from(png.data, 'binary');
            res.contentType('image/jpeg');
            res.send(img);
...
require('express-async-errors'); // Set up the imagery providers
